class ZCL_RS_3PRJABRSTAT definition
  public
  final
  create public
...
public section.
  ...
  types TS_PRJABRST type ZRS_3TT_PRJABRST_S .
  types:
    tt_prjabrst TYPE STANDARD TABLE OF ts_prjabrst WITH DEFAULT KEY .
  types:
    BEGIN OF ts_input_parameters,
        ...
        er_entity                      TYPE ts_prjabrst,
        et_entityset                   TYPE tt_prjabrst,
  ...
  data:
    gt_status TYPE RANGE OF ts_prjabrst-status .
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      ...
      END OF mc_error_opt,
      BEGIN OF mc_key_fields,
        sprache TYPE string VALUE 'SPRACHE',
        process TYPE string VALUE 'PROZESSART',
        estat   TYPE string VALUE 'ESTAT',
    ...
    BEGIN OF mc_filter_values,
        ...
        to_read_stat   TYPE string VALUE '( Sprache eq `&` and Prozessart eq `@` and Status eq `*` )' ##NO_TEXT,
        to_read_multstat   TYPE string VALUE '( Sprache eq `&` and Prozessart eq `@` and Status IN gt_status )' ##NO_TEXT,
...
CLASS ZCL_RS_3PRJABRSTAT IMPLEMENTATION.
...
*         ...
*----------------------------------------------------------------------*
    DATA(lo_katalog) = NEW zcl_rs_3prjabrstat( is_params ).
...
*----------------------------------------------------------------------*
    ...
    get_data( ).
    DATA(lo_response) = NEW zcl_rs_3prjabrstat( gs_params ).
